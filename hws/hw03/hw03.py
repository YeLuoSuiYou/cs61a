...
def check_par():
    ...
    True
    """
    r1 = interval(1, 2) # Replace this linf!
...
def quadratic(x, a, b, c):
    ...
    values = []
    values.append(quadratic_helper(lower_bound(x)))
    values.append(quadratic_helper(upper_bound(x)))

    sp = -b / (2 * a)
    if sp >= lower_bound(x) and sp <= upper_bound(x):
        values.append(quadratic_helper(sp))
