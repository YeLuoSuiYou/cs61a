...
def swine_align(player_score, opponent_score):
    ...
    if gcd(player_score, opponent_score) >= 10 and opponent_score >= 10:
        return True
    else:
        return False
    # END PROBLEM 4a
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    def play_extra_turn(strategy, player, player_score, opponent_score, say=silence):
        ...
        while(extra_turn(new_player_score, opponent_score)):
            new_player_score += take_turn(strategy(new_player_score,
                                          opponent_score), opponent_score, dice)
            if player == 0:
                say = say(new_player_score, opponent_score)
            elif player == 1:
                say = say(opponent_score, new_player_score)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    while(n <= 10):
        score = average_roll_dice(n, dice) 
        if(score > high_score):
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    with_free_bacon = bacon_point + score
