...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    valid_paraguraphs = [p for p in paragraphs if select(p)]
    if k >= len(valid_paraguraphs):
        return ''
    else:
        return valid_paraguraphs[k]
    # END PROBLEM 1
...
def about(topic):
    ...
    def about_helper(s):
        valid_s = split(lower(remove_punctuation(s)))
        for t in topic:
            if t in valid_s:
                return True
        return False
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    type_char_count = len(typed)
    typed_word_count = type_char_count / 5
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        return user_word
    possible_word = min(
        valid_words, key=lambda x: diff_function(x, user_word, limit))
...
def report_progress(typed, prompt, user_id, send):
    ...
    messege = {}
    messege['id'], messege['progress'] = user_id, progress
